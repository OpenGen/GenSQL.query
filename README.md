...
By default, results are returned in the order that they were provided (or generated by the model in the case of virtual data — see `GENERATE` below). `ORDER BY` behaves as it does in SQL. If `ORDER BY` is provided without either `ASC` or `DESC`, results are returned in ascending order.
